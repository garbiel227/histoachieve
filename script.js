...
const els = {
  ...
  shareWork: document.querySelector("#shareWork"),
...
function markIosSafari() {
  ...
  document.body.classList.toggle("is-ios-safari", isIos && isSafari);
}
...
function revealCollection(refresh = false) {
  if (cardsRevealed && !refresh) return;
  cardsRevealed = true;
  els.resultLine.classList.add("is-visible");
  observeCards();
}
...
function syncToolbarVisibility() {
  if (!els.toolbarShell || !els.classes) return;
  const shellTop = els.toolbarShell.getBoundingClientRect().top;
  const classesTop = els.classes.getBoundingClientRect().top;
  const shouldHide = classesTop <= 120;
  const isStuck = window.scrollY > 8 && shellTop <= 11 && !shouldHide;
  els.toolbarShell.classList.toggle("is-hidden", shouldHide);
  els.toolbarShell.classList.toggle("is-stuck", isStuck);
  document.body.classList.toggle("has-top-blur", isStuck);
}

function prepareCollectionRevealOnScroll() {
  ...
  window.addEventListener("resize", syncToolbarVisibility);
}
...
function updateShareMeta(work) {
  document.title = `${fullTitle(work)}｜天津师范大学新闻学系新闻史数据库`;
  const description = work.summary || `${work.grade} 班 ${work.genre}作品`;
  const tags = {
    description,
    "og:title": fullTitle(work),
    "og:description": description,
    "og:url": getSharePageUrl(work),
    "twitter:title": fullTitle(work),
...
function setViewer(work, options = {}) {
  activeWork = work;
  els.shareStatus.textContent = "";
  els.viewerGrade.textContent = `${work.grade} 班 · ${work.genre} · ${work.medium}`;
  els.viewerTitle.textContent = fullTitle(work);
  els.viewerAuthor.textContent = work.author ? `作者：${work.author}` : "";
  els.viewerDesc.innerHTML = `${work.entryDate ? `<span class="viewer-date">入库日期：${work.entryDate}</span>` : ""}${work.summary}`;
  ...
  updateShareMeta(work);
  if (options.updateUrl !== false) {
    window.history.replaceState({ workId: work.id }, "", getWorkUrl(work));
  }
  els.viewer.showModal();
}

async function shareActiveWork() {
  ...
  try {
    if (navigator.share) {
      await navigator.share({
        title: fullTitle(activeWork),
        text: activeWork.summary || "天津师范大学新闻学系新闻史数据库作品",
        url: shareUrl
      });
      els.shareStatus.textContent = "已打开微信卡片分享";
      return;
    }

    await navigator.clipboard.writeText(shareUrl);
    els.shareStatus.textContent = "卡片链接已复制，可粘贴给微信好友";
  } catch (error) {
    try {
      await navigator.clipboard.writeText(shareUrl);
      els.shareStatus.textContent = "分享面板未完成，卡片链接已复制";
    } catch (clipboardError) {
      els.shareStatus.textContent = `请手动复制卡片链接：${shareUrl}`;
    }
  }
}
...
els.shareWork.addEventListener("click", shareActiveWork);
...
renderClasses();
applyFilters();
prepareCollectionRevealOnScroll();

const initialWorkId = new URLSearchParams(window.location.search).get("work");
const initialWork = works.find((work) => work.id === initialWorkId);
if (initialWork) {
  setViewer(initialWork, { updateUrl: false });
}
