...
async function shareActiveWork() {
  ...
  const shareData = {
    ...
    url: getWorkUrl(activeWork)
  };

  try {
    if (navigator.share) {
      await navigator.share(shareData);
      els.shareStatus.textContent = "已打开分享面板";
      return;
    }

    await navigator.clipboard.writeText(shareData.url);
    els.shareStatus.textContent = "链接已复制，可粘贴到微信分享";
  } catch (error) {
    els.shareStatus.textContent = "分享失败，可复制浏览器地址栏链接";
  }
}
